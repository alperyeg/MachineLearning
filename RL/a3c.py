...
from subproc_vec import SubprocVecEnv
...
def perform_updates(network, optm):
    ...
    for i in network.rewards_achieved[::-1]:
        ...
        r = GAMMA * r + i
        rewards.insert(0, r)
    returns = torch.from_numpy(np.array(rewards))
    ...
    for (log_prob, val), R in zip(saved_action, returns):
        ...
        # value loss
        critic_losses.append(F.mse_loss(val.float(), R.float()))
    optm.zero_grad()
...
if __name__ == '__main__':
    ...
    for idx in range(EPISODES):
        ...
        while not np.any(done):
            ...
            state, reward, done, _ = envs.step(action.cpu().numpy())  # cpu().numpy() ?
